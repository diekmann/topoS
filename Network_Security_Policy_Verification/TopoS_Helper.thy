...
imports Main TopoS_Interface TopoS_Util 
  TopoS_ENF
  TopoS_withOffendingFlows_lemmata
...
lemma (in TopoS_preliminaries) eval_model_valid_remove_flattened_offending_flows:
  ...
  from remove_offending_flows_imp_model_valid have 1:
    "\<forall>f\<in>set_offending_flows \<lparr>nodes = nodesG, edges = edgesG\<rparr> nP. eval_model \<lparr>nodes = nodesG, edges = edgesG - f \<rparr> nP"
      ...
      by (metis TopoS_withOffendingFlows.valid_without_offending_flows delete_edges_simp2 graph.select_convs(1) graph.select_convs(2))
