theory NetworkModel_Stateful_Policy_Algorithm
imports NetworkModel_Stateful_Policy NetworkModel_Composition_Theory
begin
...
section {* Sketch for generating a stateful policy from a simple directed policy *}
  ...
  proof -
    ...
    have validG': "valid_graph \<lparr>nodes = nodes G, edges = edges G \<union> set (filter_IFS_no_violations G M edgesList)\<rparr>" 
      by (metis NetworkModel_withOffendingFlows.graph_eq_intro Un_absorb2 graph.select_convs(1) graph.select_convs(2) order.trans)
