...
and verify_globals = verify_globals
  apply unfold_locales
   ...
   apply(simp add: TopoS_withOffendingFlows.is_offending_flows_def graph_ops split: split_split split_split_asm)[1]
...
lemma BLP_def_unique: "otherbot \<noteq> default_node_properties \<Longrightarrow>
    \<exists>G p i f. valid_graph G \<and> \<not> eval_model G p \<and> f \<in> (TopoS_withOffendingFlows.set_offending_flows eval_model G p) \<and>
...
section {*ENF*}
  ...
  lemma BLP_ENF: "TopoS_withOffendingFlows.eval_model_all_edges_normal_form eval_model BLP_P"
    unfolding TopoS_withOffendingFlows.eval_model_all_edges_normal_form_def
    unfolding BLP_P_def
    by simp
  lemma BLP_ENF_refl: "TopoS_withOffendingFlows.ENF_refl eval_model BLP_P"
  ...
  lemma BLP_offending_set: "TopoS_withOffendingFlows.set_offending_flows eval_model = BLP_offending_set"
    apply(simp only: fun_eq_iff TopoS_withOffendingFlows.ENF_offending_set[OF BLP_ENF] BLP_offending_set_def)
    apply(rule allI)+
...
interpretation BLPtrusted: TopoS_IFS 
  ...
  and verify_globals = verify_globals
  where "TopoS_withOffendingFlows.set_offending_flows eval_model = BLP_offending_set"
    apply unfold_locales
      apply(rule ballI)
      apply (rule_tac f=f in TopoS_withOffendingFlows.ENF_snds_refl_instance[OF BLP_ENF_refl zero_default_candidate])
       apply(simp)
      apply(simp)
  ...
  lemma TopoS_BLPtrusted: "NetworkModel eval_model default_node_properties target_focus"
