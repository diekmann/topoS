...
lemma sinvar_correct: "valid_list_graph G \<Longrightarrow> NM_Dependability.sinvar (list_graph_to_graph G) nP = sinvar G nP"
   apply(simp)
...
interpretation Dependability_impl:TopoS_List_Impl 
  ...
  and sinvar_spec=NM_Dependability.sinvar
...
section {* Dependability packing *}
  ...
  interpretation NM_LIB_Dependability_interpretation: TopoS_modelLibrary NM_LIB_Dependability
      NM_Dependability.sinvar NM_Dependability.verify_globals
...
text {*Example: *}
  value "let G = \<lparr> nodesL = [1::nat,2,3,4,8,9,10], edgesL = [(1,2), (2,3), (3,4), (8,9),(9,8)] \<rparr>
      in sinvar G  ((\<lambda> n. NM_Dependability.default_node_properties)(1:=3, 2:=2, 3:=1, 4:=0, 8:=2, 9:=2, 10:=0))"
  
  value "let G = \<lparr> nodesL = [1::nat,2,3,4,8,9,10], edgesL = [(1,2), (2,3), (3,4), (8,9),(9,8)] \<rparr>
      in sinvar G  ((\<lambda> n. NM_Dependability.default_node_properties)(1:=10, 2:=10, 3:=10, 4:=10, 8:=10, 9:=10, 10:=10))"
