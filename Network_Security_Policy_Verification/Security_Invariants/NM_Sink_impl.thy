theory NM_Sink_impl
imports NM_Sink "../TopoS_Lists_Impl_Interface"
begin
...
interpretation Sink_impl:TopoS_List_Impl 
  ...
  and sinvar_spec=NM_Sink.sinvar
...
section {* Sink packing *}
  definition NM_LIB_Sink :: "('v::vertex, node_config, unit) TopoS_packed" where
    ...
    \<lparr> nm_name = ''Sink'', 
      ...
      nm_eval = Sink_eval
      \<rparr>"
  interpretation NM_LIB_Sink_interpretation: TopoS_modelLibrary NM_LIB_Sink
      NM_Sink.sinvar NM_Sink.verify_globals
    apply(unfold TopoS_modelLibrary_def NM_LIB_Sink_def)
    apply(rule conjI)
     apply(simp)
    apply(simp)
    by(unfold_locales)
