...
subsubsection {*Preliminaries*}
 ...
 lemma NoRfl_ENRsr: "TopoS_withOffendingFlows.eval_model_all_edges_normal_form_sr eval_model (\<lambda> nP\<^sub>s s nP\<^sub>r r. s = r \<longrightarrow> nP\<^sub>s = Refl)"
    by(simp add: TopoS_withOffendingFlows.eval_model_all_edges_normal_form_sr_def)
 ...
 thm TopoS_withOffendingFlows.ENFsr_offending_set[OF NoRfl_ENRsr]

  lemma NoRefl_offending_set: "TopoS_withOffendingFlows.set_offending_flows eval_model = NoRefl_offending_set"
    apply(simp only: fun_eq_iff NoRefl_offending_set_def)
    apply(rule allI)+
    apply(rename_tac G nP)
    apply(simp only: TopoS_withOffendingFlows.ENFsr_offending_set[OF NoRfl_ENRsr])
...
and verify_globals = verify_globals
where "TopoS_withOffendingFlows.set_offending_flows eval_model = NoRefl_offending_set"
  ...
  lemma TopoS_NoRefl: "NetworkModel eval_model default_node_properties target_focus"
