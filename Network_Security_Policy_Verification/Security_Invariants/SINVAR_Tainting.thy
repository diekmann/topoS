...
type_synonym taints = "string set" (*TODO: hide synonym*)

definition default_node_properties :: "taints"
...
lemma sinvar_tainting_edges_def: "wf_graph G \<Longrightarrow>
...
lemma "let G = \<lparr>nodes = {1::nat, 2}, edges = {(1, 1), (1, 2), (2, 2)}\<rparr>;
           nP = (\<lambda>x. default_node_properties)(1 := {}, 2 := {''foo''})
       in sinvar_tainting G nP"
oops

text{*Alternative definition of the @{const sinvar_tainting}*}

definition sinvar :: "'v graph \<Rightarrow> ('v \<Rightarrow> taints) \<Rightarrow> bool" where
  "sinvar G nP \<equiv> \<forall> (v1,v2) \<in> edges G. nP v1 \<subseteq> nP v2"


lemma sinvar_preferred_def:
...
lemma tainting_def_unique: "otherbot \<noteq> {} \<Longrightarrow>
