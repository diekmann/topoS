...
// Generated by Isabelle (Isabelle2013-1: November 2013) on Mon Nov 18 14:11:13 2013
// src: NetworkModel_generateCode.thy
...
object NM_DomainHierarchyNG {

import /*implicits*/ String.equal_char

abstract sealed class domainNameDept
final case class Dept(a: List[Char], b: domainNameDept) extends domainNameDept
final case class Leaf() extends domainNameDept

abstract sealed class domainNameTrust
final case class Dn(a: (domainNameDept, Nat.nat)) extends domainNameTrust
final case class Unassigned() extends domainNameTrust
...
def domainNameDeptChopOne(x0: domainNameDept): domainNameDept = x0 match {
  case Leaf() => Leaf()
  case Dept(name, Leaf()) => Leaf()
  case Dept(name, Dept(v, va)) => Dept(name, domainNameDeptChopOne(Dept(v, va)))
}

def domainNameChop(x0: domainNameDept, uu: Nat.nat): domainNameDept = (x0, uu)
  match {
  case (Leaf(), uu) => Leaf()
  case (Dept(v, va), n) =>
    (if (Nat.equal_nat(n, Nat.zero_nat)) Dept(v, va)
      else domainNameChop(domainNameDeptChopOne(Dept(v, va)),
                           Nat.minus_nat(n, Nat.one_nat)))
}

def hierarchy_next(x0: List[domainTree], uu: domainNameDept):
      Option[domainTree] =
  (x0, uu) match {
  case (Nil, uu) => None
  case (s :: ss, Leaf()) => None
...
def less_eq_domainNameDept(x0: domainNameDept, x1: domainNameDept): Boolean =
  (x0, x1) match {
  case (Leaf(), Dept(uu, uv)) => false
  case (Dept(uw, ux), Leaf()) => true
  case (Leaf(), Leaf()) => true
  case (Dept(n1, n1s), Dept(n2, n2s)) =>
    Lista.equal_list[Char](n1, n2) && less_eq_domainNameDept(n1s, n2s)
}

def leq_domainNameTrust(x0: domainNameTrust, uu: domainNameTrust): Boolean =
  (x0, uu) match {
  case (Unassigned(), uu) => true
  case (Dn(v), Unassigned()) => false
  case (Dn((dnA, trustA)), Dn((dnB, trustB))) =>
    less_eq_domainNameDept(dnA, domainNameChop(dnB, trustB))
}

def eval_model[A](g: FiniteListGraph.list_graph_ext[A, Unit],
                   nP: A => domainNameTrust):
      Boolean =
  Lista.list_all[(A, A)]((a: (A, A)) =>
                           {
                             val (s, r): (A, A) = a;
                             leq_domainNameTrust(nP(r), nP(s))
                           },
                          FiniteListGraph.edgesL[A, Unit](g))

def valid_hierarchy_pos(x0: domainTree, x1: domainNameDept): Boolean = (x0, x1)
  match {
  case (Department(d, ds), Leaf()) => true
...
def verify_globals[A](g: FiniteListGraph.list_graph_ext[A, Unit],
                       nP: A => domainNameTrust, tree: domainTree):
      Boolean =
  Lista.list_all[A]((v: A) =>
                      (nP(v) match {
                         case Dn((level, _)) => valid_hierarchy_pos(tree, level)
                         case Unassigned() => true
                       }),
                     FiniteListGraph.nodesL[A, Unit](g))

def default_node_properties: domainNameTrust = Unassigned()
...
def netModel_node_props[A : NetworkModel_Vertices.vertex, B,
      ...
      A => domainNameTrust =
  (i: A) =>
    ((NetworkModel_Interface.node_properties[A, domainNameTrust, B,
      C](p)).apply(i)
       match {
       case None => default_node_properties
       case Some(property) => property
...
def domainHierarchyNG_eval[A : HOL.equal : NetworkModel_Vertices.vertex : Orderings.linorder](g:
                FiniteListGraph.list_graph_ext[A, Unit],
               p: NetworkModel_Interface.networkModel_Params_ext[A,
                          domainNameTrust, domainTree, Unit]):
      Boolean =
  FiniteListGraph.valid_list_graph[A](g) &&
    (verify_globals[A](g, netModel_node_props[A, domainTree, Unit](p),
                        NetworkModel_Interface.model_global_properties[A,
                                domainNameTrust, domainTree, Unit](p)) &&
      eval_model[A](g, netModel_node_props[A, domainTree, Unit](p)))

def domainHierarchyNG_offending_list[A](g:
  FiniteListGraph.list_graph_ext[A, Unit],
 nP: A => domainNameTrust):
      List[List[(A, A)]] =
  (if (eval_model[A](g, nP)) Nil
...
def nM_LIB_DomainHierarchyNG[A : HOL.equal : NetworkModel_Vertices.vertex : Orderings.linorder]:
      NetworkModel_Lists_Impl_Interface.networkModel_packed_ext[A,
                         domainNameTrust, domainTree, Unit] =
  NetworkModel_Lists_Impl_Interface.NetworkModel_packed_ext[domainNameTrust, A,
                     domainTree,
                     Unit](List('D', 'o', 'm', 'a', 'i', 'n', 'H', 'i', 'e',
                                 'r', 'a', 'r', 'c', 'h', 'y', 'N', 'G'),
                            target_focus, default_node_properties,
                            (a: FiniteListGraph.list_graph_ext[A, Unit]) =>
                              (b: A => domainNameTrust) => eval_model[A](a, b),
                            (a: FiniteListGraph.list_graph_ext[A, Unit]) =>
                              (b: A => domainNameTrust) =>
                                (c: domainTree) => verify_globals[A](a, b, c),
                            (a: FiniteListGraph.list_graph_ext[A, Unit]) =>
                              (b: A => domainNameTrust) =>
                                domainHierarchyNG_offending_list[A](a, b),
                            (a: NetworkModel_Interface.networkModel_Params_ext[A,
domainNameTrust, domainTree, Unit])
                              => netModel_node_props[A, domainTree, Unit](a),
                            (a: FiniteListGraph.list_graph_ext[A, Unit]) =>
                              (b: NetworkModel_Interface.networkModel_Params_ext[A,
  domainNameTrust, domainTree, Unit])
                                => domainHierarchyNG_eval[A](a, b),
                            ())
